...
@interface SHUImagePicker()
    <UIImagePickerControllerDelegate, UINavigationControllerDelegate, SHUCropImageControllerDelegate>
...
- (void) showPickerInViewController:(UIViewController *)viewController
    ...
    self.imagePickerController.navigationBar.barTintColor = [UIColor colorWithRed:
            68.0 / 255.0
            green:68.0 / 255.0
            blue:68.0 / 255.0
            alpha:1.0];
...
- (void) _presentImagePickerControllerFromRect:(CGRect) rect {
    ...
    if (SYSTEM_VERSION_LESS_THAN(@"8.0")) {
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
            ...
            [self.popoverController presentPopoverFromRect:rect inView:
             self.targetViewController.view permittedArrowDirections:
...
- (void) _presentImagePickerControllerFromBarButtonItem:
    ...
    if (SYSTEM_VERSION_LESS_THAN(@"8.0")) {
        ...
        self.imagePickerController.modalPresentationStyle = UIModalPresentationPopover;
        self.imagePickerController.popoverPresentationController.barButtonItem =
            barButtonItem;
        [self.targetViewController presentViewController:self.imagePickerController
         animated:YES completion:nil];
    }
...
- (void) _performDrillInWithPhoto:(UIImage *)photo pickerController:
    (UIImagePickerController *)picker {
    SHUCropImageController *cropViewController = [[SHUCropImageController alloc]
            initWithNibName:@"SHUCropImageController"
...
- (void) cropViewControllerDidCropImage:(UIImage *)cropedImage {
    ...
    [self.targetViewController.presentedViewController
